...
export default function CarPanel() {
    ...
    return (
        <div>
            ...
            <button className="block rounded-md bg-sky-600 hover:bg-indigo-600 px-3 py-2 text-white " onClick={()=>{countRef.current=countRef.current+1; alert(countRef.current)}}>
                Count
            </button>
            ...
            ref={inputRef}/>
            <button className="block rounded-md bg-sky-600 hover:bg-indigo-600 px-3 py-2 text-white " onClick={()=>{if(inputRef.current!=null) inputRef.current.focus()}}>
                Focus Input
            </button>
        </div>
    )
